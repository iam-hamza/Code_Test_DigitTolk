...
class BookingRepository extends BaseRepository
{
   ...
   /**
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if (!$cuser) {
            return ['emergencyJobs' => [], 'normalJobs' => [], 'cuser' => null, 'userType' => 'unknown'];
        }

        $usertype = $cuser->is('customer') ? 'customer' : ($cuser->is('translator') ? 'translator' : 'unknown');
        
        $emergencyJobs = $this->getEmergencyJobs($cuser);
        $normalJobs = $this->getNormalJobs($cuser, $user_id);

        return compact('emergencyJobs', 'normalJobs', 'cuser', 'userType');
    }

    /**
     * Get emergency jobs for the user.
     *
     * @param User $user
     * @return array
     */
    private function getEmergencyJobs(User $user)
    {
        $jobs = $this->getJobsForUser($user);

        return $this->filterJobsByType($jobs, 'yes');
    }

    /**
     * Get normal jobs for the user.
     *
     * @param User $user
     * @param int $user_id
     * @return array
     */
    private function getNormalJobs(User $user, int $user_id)
    {
        $jobs = $this->getJobsForUser($user);

        return $this->filterJobsByType($jobs, 'no')
     ...
     */
    private function getJobsForUser(User $user)
    {
        if ($user->is('customer')) {
    ...
    /**
     * Filter jobs by immediate type.
     *
     * @param \Illuminate\Support\Collection $jobs
     * @param string $type
     * @return \Illuminate\Support\Collection
     */
    private function filterJobsByType($jobs, $type)
    {
        return $jobs->filter(function ($job) use ($type) {
            return $job->immediate === $type;
        });
    }
...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        // Handle any additional logic or events here
        return $response;
    }

    /**
     * Store user email and update job details.
     *
     * @param array $data
     * @return array
     */
    public function storeJobEmail(array $data)
    {
        ...
        // Find the job by ID or fail if not found
        ...
        if (isset($data['address'])) {
            ...
            // Update job address, instructions, and town based on the provided data or user's defaults
        ...
        // Determine the email and name for sending
        $email = !empty($userEmail) ? $userEmail : $user->email;
        $name = $user->name;
   ...
   /**
     * Convert a job model to an array of data for Push notifications.
     *
     * @param $job
     * @return array
     */
    public function jobToData($job)
    {
        ...
        $data['job_for'] = $this->formatJobFor($job);

        return $data;
    }

    /**
     * Format the "job_for" field based on gender and certified values.
     *
     * @param $job
     * @return array
     */
    ...
    /**
     * End a job and send email notifications.
     *
    ...
    public function jobEnd($post_data = [])
    {
        ...
        if (!$job) {
            return; // Job not found, handle this case accordingly
        }
        ...
        $this->sendSessionEndedEmail($email, $name, $job, $sessionTime, 'faktura');
        ...
        if ($translatorJobRel) {
            ...
            $this->sendSessionEndedEmail($email, $name, $job, $sessionTime, 'lön');
        ...
        $this->triggerSessionEndedEvent($job, $translatorJobRel, $post_data);
    }

    /**
     * Calculate the session time difference between two dates.
     *
     * @param string $completedDate
     * @param string $dueDate
     * @return string
     */
    private function calculateSessionTime($completedDate, $dueDate)
    {
        $start = now()->parse($dueDate);
        $end = now()->parse($completedDate);
        $diff = $end->diff($start);
     ...
     */
    private function sendSessionEndedEmail($email, $name, $job, $sessionTime, $forText)
    {
    ...
    /**
     * Trigger the session ended event.
     *
     * @param Job $job
     * @param TranslatorJobRel|null $translatorJobRel
     * @param array $post_data
     */
    private function triggerSessionEndedEvent($job, $translatorJobRel, $post_data)
    {
        $userId = $post_data['userid'];
        $recipientUserId = ($userId == $job->user_id) ? $translatorJobRel->user_id : $job->user_id;
   ...
   /**
     * Get potential job IDs for a user based on their profile and preferences.
     *
     * @param int $user_id
     * @return array
     */
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        // Set the default job type based on translator type
        $job_type = ($translator_type == 'professional') ? 'paid' : 'unpaid';
        ...
        // Retrieve potential job IDs based on user preferences
        ...
        $filtered_job_ids = $this->filterJobsByCriteria($job_ids, $user_id);
    ...
    /**
     * Filter job IDs based on additional criteria.
     *
     * @param array $job_ids
     * @param int $user_id
     * @return array
     */
    private function filterJobsByCriteria($job_ids, $user_id)
    {
        foreach ($job_ids as $k => $v) {
            $job = Job::find($v->id);
            $job_userid = $job->user_id;
            $check_town = Job::checkTowns($job_userid, $user_id);

            // Check additional criteria and remove jobs that don't match
            if (
